...
(async()=>{
 const root=path.resolve(__dirname,'..'),base=process.env.BASE_URL||'http://127.0.0.1:8767/';
 ...
 await page.goto(new URL('greeks.html',base).href,{waitUntil:'networkidle'});
 assert.ok(await page.locator('#calendar-greeks-lab .lab').isVisible());
 const ranges=page.locator('#calendar-greeks-lab input[type="range"]');
 await ranges.nth(3).fill('60');
 await ranges.nth(2).fill('120');
 assert.equal(await ranges.nth(3).inputValue(),'121','Long maturity must show the value used in calculations');
 assert.equal(await ranges.nth(3).getAttribute('min'),'121');
 assert.ok((await page.locator('#calendar-greeks-lab').innerText()).includes('pp'),'Volatility spread must use percentage points');
 await browser.close();
 assert.deepEqual(errors,[],`Console errors:\n${errors.join('\n')}`);
 console.log(`Checked ${pages.length} pages at desktop and mobile, search, theme and Greek lab.`);
