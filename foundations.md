...
ตลาดอนุพันธ์ (derivatives market) มีปัจจัยหลายด้านที่ส่งผลต่อราคาออปชัน (option pricing) กลยุทธ์ Calendar Spread ใช้ความแตกต่างของการเสื่อมมูลค่าเวลา (time value) และความผันผวนแฝง (implied volatility หรือ IV) ระหว่างสัญญาออปชันที่มีวันหมดอายุต่างกัน ผลลัพธ์ขึ้นอยู่กับเส้นทางราคา การเปลี่ยนแปลง IV ของแต่ละขา และต้นทุนการซื้อขาย ไม่ใช่ผลตอบแทนที่สม่ำเสมอโดยอัตโนมัติ ([Options Industry Council](https://www.optionseducation.org/strategies/all-strategies/long-call-calendar-spread-call-horizontal)).
...
บทเรียนนี้ใช้คำว่า Volatility Risk Premium (VRP) อย่างง่ายสำหรับส่วนต่าง $IV-RV$ โดย IV คือความผันผวนที่สะท้อนในราคาออปชัน และ RV คือความผันผวนที่เกิดขึ้นจริงในช่วงที่พิจารณา ควรเทียบทั้งสองค่าด้วยช่วงเวลาและหน่วยรายปีเดียวกัน นิยามนี้ต่างจาก **variance risk premium** ซึ่งใช้อัตราความแปรปรวน เช่นในงาน [Carr and Wu (2009)](https://doi.org/10.1093/rfs/hhn038) เมื่อ RV สูงกว่า IV ส่วนต่างของบทเรียนจะติดลบ การที่ IV พุ่งขึ้นเพียงอย่างเดียวจึงไม่ได้ทำให้ส่วนต่างกลับเครื่องหมาย.
...
ภายใต้สมมติฐาน BSM ราคา $V(S,t)$ ของออปชัน European บนสินทรัพย์ที่ไม่จ่ายเงินปันผลเป็นผลเฉลยของสมการเชิงอนุพันธ์ย่อยแบล็ก-โชลส์: $$\begin{equation}
...
\end{equation}$$ เมื่อพจน์ดอกเบี้ยและ Delta มีผลน้อย Positive Gamma จึงมาพร้อมกับ Negative Theta โดยประมาณ และในทางกลับกัน ความสัมพันธ์นี้อธิบายการแลกเปลี่ยน (trade-off) ของพอร์ตภายใต้สมมติฐานเดียวกัน แต่ไม่ใช่ข้อสรุปที่ใช้ได้ทุกพอร์ตหรือทุกโครงสร้าง IV.
...
<table>
<caption><strong>ตารางที่ 2.</strong> เปรียบเทียบสมการอนุพันธ์ย่อยและ <span class="math inline"><em>Θ</em></span> ของแต่ละแบบจำลอง</caption>
<thead>
<tr>
