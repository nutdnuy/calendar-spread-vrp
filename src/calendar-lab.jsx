...
export function CalendarGreeksLab(){
  ...
  const params={spot,strike,rate:rate/100,shortVol:shortIV/100,longVol:longIV/100,shortTime:shortDays/365,longTime:Math.max(longDays,shortDays+1)/365};
  ...
  return <section className="lab" aria-label="ห้องทดลองค่ากรีกสุทธิของ Calendar Spread">
    ...
    <div className="controls two"><div>
      <Range label="Spot" value={spot} onChange={setSpot} min={60} max={140}/><Range label="Strike" value={strike} onChange={setStrike} min={60} max={140}/><Range label="Short maturity" value={shortDays} onChange={setShortDays} min={7} max={120} suffix=" days"/><Range label="Long maturity" value={longDays} onChange={setLongDays} min={60} max={365} suffix=" days"/>
    </div><div>
